...
def test_fetch_config_structure(tmpdir):
    ...
    # Then
    assert config.gradient_boosting_model_config
    assert config.app_config

    # Validate model_config fields
    assert hasattr(config.gradient_boosting_model_config, 'drop_features')
    assert hasattr(config.gradient_boosting_model_config, 'target')
    assert hasattr(config.gradient_boosting_model_config, 'n_estimators')
    assert config.gradient_boosting_model_config.drop_features == 'YrSold'
    assert config.gradient_boosting_model_config.target == 'SalePrice'

    # Validate app_config fields
