...
def test_sklearn_transformer_wrapper_with_config_numerical_vars(pipeline_inputs):
    ...
    assert all(var in X_train.columns for var in numerical_vars)
    ...
    for var in numerical_vars:
        # Ensure NaNs are imputed for all numerical variables
        assert not X_transformed[var].isna().any()
...
def test_drop_unnecessary_features_transformer(pipeline_inputs):
    ...
    assert isinstance(drop_features, list), (
        f"drop_features should be a list, got {type(drop_features)}"
    )
    ...
    for feature in drop_features:
        assert feature not in X_transformed.columns, (
            f"Feature '{feature}' was not dropped"
        )

    # Additionally, ensure no other features were unintentionally dropped or modified
    for column in X_train.columns:
        if column not in drop_features:
            assert column in X_transformed.columns, (
                f"Feature '{column}' was unintentionally dropped or modified"
            )

    # Check if the transformed DataFrame still contains the same number of rows
    assert X_train.shape[0] == X_transformed.shape[0], (
        "Number of rows has changed after dropping features"
    )
...
def test_temporal_variable_estimator(pipeline_inputs):
    ...
    assert all(var in X_train.columns for var in temporal_vars), (
        f"Missing temporal variables: {temporal_vars}"
    )
    ...
    for i in range(X_train.shape[0]):
        for var in temporal_vars:
            expected_difference = (
                    X_train.iloc[i][reference_variable]
                    - X_train.iloc[i][var]
            )
    ...
    assert X_train.shape[0] == X_transformed.shape[0], (
        "Number of rows has changed after transformation"
    )
